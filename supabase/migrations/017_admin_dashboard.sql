-- 017_admin_dashboard.sql
-- Admin dashboard aggregation RPCs (real data, no mock).
-- SECURITY DEFINER + explicit is_admin() guard — mirrors the pattern from 009.
-- No service_role key ever touches the browser.

-- ============ إحصائيات لوحة التحكم الشاملة (أدمن فقط) ============
...
begin
  ...
  recent := (
    ...
    from (
      select * from (
        select 'research' as entity, id, slug, title_ar, title_en, status, published_at, updated_at
          from public.research_papers
        union all
        select 'publication', id, slug, title_ar, title_en, status, published_at, updated_at
          from public.publications
        union all
        select 'supervision', id, slug, title_ar, title_en, status, published_at, updated_at
          from public.scientific_supervisions
        union all
        select 'discussion', id, slug, title_ar, title_en, status, published_at, updated_at
          from public.scientific_discussions
        union all
        select 'project', id, slug, title_ar, title_en, status, published_at, updated_at
          from public.research_projects
        union all
        select 'course', id, slug, title_ar, title_en, status, published_at, updated_at
          from public.courses
        union all
        select 'lecture', id, slug, title_ar, title_en, status, published_at, updated_at
          from public.lectures
        union all
        select 'axis', id, slug, name_ar, name_en, status, null::timestamptz, updated_at
          from public.scientific_axes
        union all
        select 'insight', id, slug, title_ar, title_en, status, published_at, updated_at
          from public.scientific_insights
        union all
        select 'news', id, slug, title_ar, title_en, status, published_at, updated_at
          from public.news
        union all
        select 'interest', id, null::text, title_ar, title_en, status, null::timestamptz, updated_at
          from public.research_interests
        union all
        select 'calendar', id, null::text, title_ar, title_en, status, null::timestamptz, updated_at
...
-- ============ سلسلة زمنية للتفاعل والنشر (أدمن فقط) ============
  ...
  published := (
    ...
    left join (
      select day, sum(n) as n from (
        ...
        union all select published_at::date, count(*) from public.research_interests where published_at is not null group by published_at::date
        union all select published_at::date, count(*) from public.calendar_events where published_at is not null group by published_at::date
      ) x
      group by day
    ) p on p.day = d
  );
