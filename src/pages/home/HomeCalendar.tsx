...
export function HomeCalendar() {
  ...
  return (
    <div className="rounded-xl border border-[#E7DFED] bg-white p-3.5 shadow-[0_1px_4px_rgb(53_20_92_/_0.05)]">
      <div className="mb-2 flex items-center gap-2 border-b border-slate-100 pb-2">
        <CalendarIcon className="h-4 w-4 text-[#D89A16]" />
        <h3 className="font-display text-[15px] font-extrabold text-[#35145C]">{t('home.calendarTitle')}</h3>
      </div>

      <div className="mb-2 flex items-center justify-between px-1 text-xs font-bold text-[#35145C]">
        <button
          type="button"
          onClick={() => setCursor((c) => new Date(c.getFullYear(), c.getMonth() - 1, 1))}
          className="text-[#D89A16] transition-colors hover:text-[#B9780E]"
      ...
      <div className="mb-1 grid grid-cols-7 text-center text-[11px] font-bold text-[#35145C]">
        {weekdays.map((day, i) => (
          <span key={i}>{day}</span>
        ))}
      </div>

      <div className="grid grid-cols-7 gap-y-0.5 text-center text-xs font-bold text-[#35145C]">
        {grid.map((date, i) => {
          ...
          return (
            <button
              ...
              className="flex flex-col items-center justify-center py-px"
            ...
            >
              <span
                className={cn(
                  ...
                  isToday(date)
                    ? 'bg-[#35145C] text-xs font-bold text-white shadow-xs'
                    : 'hover:bg-slate-100',
                )}
              >
                {date.getDate()}
              ...
              <span
                className={cn('mt-0.5 h-1 w-1 rounded-full', hasEvents ? 'bg-[#D89A16]' : 'bg-transparent')}
              />
            </button>
          );
        })}
